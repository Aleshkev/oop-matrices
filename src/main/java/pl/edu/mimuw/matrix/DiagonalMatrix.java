...
public class DiagonalMatrix extends Matrix {
  ...
  protected IDoubleMatrix doMultiplication(IDoubleMatrix other, Shape resultShape) {
    ...
    if(other instanceof DiagonalMatrix that) return fromElementwiseMerge(this, that, (a, b) -> a * b);
  ...
  protected IDoubleMatrix doAddition(IDoubleMatrix other) {
    ...
    if(other instanceof IdentityMatrix that) return fromElementwiseMerge(this, that, Double::sum);
    if(other instanceof DiagonalMatrix that) return fromElementwiseMerge(this, that, Double::sum);
    return FullMatrix.fromAddition(this, other);
  }

  @Override
  protected IDoubleMatrix doScalarOperation(DoubleFunction<Double> operator) {
    if(operator.apply(0.0) == 0.0) return fromElementwiseMerge(this, this, (a, b) -> operator.apply(a));
